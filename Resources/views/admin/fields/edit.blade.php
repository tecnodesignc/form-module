...
@section('content-header')
    <h1>
        {{ trans('form::fields.title.edit field') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i
                        class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.form.form.edit', [$form->id]) }}">{{ trans('form::forms.title.forms') }}</a></li>
        <li class="active">{{ trans('form::fields.title.edit field') }}</li>
    </ol>
@stop

@section('content')
    {!! Form::open(['route' => ['admin.form.field.update', [$form->id,$field->id]], 'method' => 'put']) !!}
                            ...
                            <div class="tab-content">
                                ...
                                @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                                    ...
                                    <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                                        @include('form::admin.fields.partials.edit-fields', ['lang' => $locale])
                                    </div>
                                @endforeach
                    ...
                    <div class="box box-primary">
                        ...
                        <div class="box-body">
                            ...
                            <div class='form-group{{ $errors->has("name") ? ' has-error' : '' }}'>
                                ...
                                @php $old = $field->name?? '' @endphp
                                {!! Form::text("name", old("name",$old), ['class' => 'form-control', 'data-slug' => 'source', 'placeholder' => trans('form::fields.form.name')]) !!}
                                {!! $errors->first("name", '<span class="help-block">:message</span>') !!}
                            </div>
                            <div class="checkbox">
                                @php $old = $field->active?? false @endphp
                                <label for="active">
                                    <input id="active"
                                           name="active"
                                           type="checkbox"
                                           class="flat-blue"
                                           {{ (is_null(old('active',$old))) ?: 'checked' }}
                                           value="1"/>
                            ...
                            <div class='form-group{{ $errors->has("order") ? ' has-error' : '' }}'>
                                ...
                                @php $old = $field->order?? 0 @endphp
                                {!! Form::number("order", old("order",$old), ['class' => 'form-control', 'data-slug' => 'source', 'placeholder' => trans('form::fields.form.order')]) !!}
