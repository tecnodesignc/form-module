...
?>
{{ csrf_field() }}
@if(Setting::get('form::trans')=="1")
    <div class="form-group">
        @if($field['type'] != "textarea" && $field['type'] != "select" && $field['type'] != "checkbox" && $field['type'] != "radio"&& $field['type']!="terms")
            <div class="col-sm-12">
                <input type="{!!$field['type']!!}" class="form-control" name="{!!$field['name']!!}" id="input{!!$field['name']!!}" required placeholder="{{trans('icustom::form.field.'.$field['name'])}}">
            </div>
        @elseif($field['type'] == "textarea")
            <div class="col-sm-12">
                <textarea class="form-control" name="{!!$field['name']!!}" placeholder="{{ trans('icustom::form.field.'.$field['name']) }}" rows="4"></textarea>
            </div>
        ...
        @elseif($field['type'] == "terms")
            <div class="checkbox col-sm-10">
                <label>
                    <input name="{!!$field['name']!!}" type="checkbox" required>{!!sprintf(trans('form::form.form.terms'),url($field['description']))!!}
                </label>
            </div>
