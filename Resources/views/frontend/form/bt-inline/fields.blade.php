    ...
    @foreach($fields as $index => $field)
        <div class="form-group">
            @if($field['type'] != "textarea" && $field['type'] != "select" && $field['type'] != "checkbox" && $field['type'] != "radio" && $field['type']!="terms")
                <label for="{!!$field['name']!!}" class="">{!!trans('icustom::form.field.'.$field['name']!!}: </label>
                <input type="{!!$field['type']!!}" class="form-control" name="{!!$field['name']!!}" id="input{!!$field['name']!!}" required placeholder="{!! trans('icustom::form.field.'.$field['description']) or '' !!}">
